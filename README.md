<div align="center">

<img src="https://i.pinimg.com/originals/77/ca/a3/77caa32884d735d439ade45ba37feaf2.gif" width="100%">

# Hi there 👋 I'm Jethzibel Abigail Rodriguez Reinoso

### Software Developer • Full Stack Developer • Software Technologist

<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=600&size=28&duration=3000&pause=900&color=22C55E&center=true&vCenter=true&width=800&lines=Software+Developer;Full+Stack+Developer;Building+Modern+Web+Applications;Creating+Digital+Solutions;Always+Learning+New+Technologies" />

<br>

*"Turning ideas into digital experiences through clean code and modern design."*

</div>

---

# 👩🏻‍💻 About Me

<img align="right" width="320" src="https://octodex.github.com/images/daftpunktocat-guy.gif">

I'm **Jethzibel Abigail Rodriguez Reinoso**, a passionate **Software Developer** focused on creating modern, scalable and user-friendly web applications.

I enjoy transforming ideas into real digital solutions by combining clean code, intuitive interfaces and efficient backend architectures.

My goal is not only to build software that works, but software that delivers an exceptional user experience.

<br>

### A little more about me

- 💚 Passionate about software development.
- 💻 Full Stack Web Developer.
- 🎓 Software Technologist.
- 🚀 Always learning new technologies.
- 🧩 I enjoy solving real-world problems.
- 📱 Interested in UI/UX and responsive design.
- ⚡ Clean code enthusiast.
- 🇨🇴 Based in Colombia.

---

# 🚀 What I Do

<table>

<tr>

<td width="50%">

## 💻 Web Development

- Responsive Websites
- Web Applications
- Landing Pages
- Dashboards
- Admin Panels

</td>

<td width="50%">

## ⚙ Backend

- REST APIs
- Authentication
- Database Integration
- CRUD Systems
- Business Logic

</td>

</tr>

<tr>

<td>

## 🗄 Database

- MySQL
- Firebase
- MongoDB

</td>

<td>

## 🎨 UI / UX

- Responsive Design
- Modern Interfaces
- User Experience
- Accessibility
- Clean Layouts

</td>

</tr>

</table>

---

# 🛠 Tech Stack

## 🌐 Frontend

<p align="center">

<img src="https://skillicons.dev/icons?i=html,css,js,bootstrap,react,tailwind"/>

</p>

---

## ⚙ Backend

<p align="center">

<img src="https://skillicons.dev/icons?i=nodejs,express,php"/>

</p>

---

## 🗄 Databases

<p align="center">

<img src="https://skillicons.dev/icons?i=mysql,mongodb,firebase"/>

</p>

---

## 🧰 Tools

<p align="center">

<img src="https://skillicons.dev/icons?i=git,github,vscode,figma,photoshop,npm"/>

</p>

---

# ⭐ Technologies I Work With

| Frontend | Backend | Database | Design |
|----------|----------|----------|----------|
| HTML5 | Node.js | Firebase | Figma |
| CSS3 | Express | MySQL | Photoshop |
| JavaScript | PHP | MongoDB | Canva |
| Bootstrap | REST APIs | Firestore | UI Design |
| React | Authentication | SQL | UX |

---

# 💼 Services

<table>

<tr>

<td>

### 🌐 Web Development

Professional websites designed for speed, responsiveness and modern user experience.

</td>

<td>

### 🛒 Business Systems

Inventory, sales, reports, customers, suppliers and administration systems.

</td>

</tr>

<tr>

<td>

### 🔥 Firebase Integration

Authentication, Firestore, Storage and cloud services.

</td>

<td>

### 🎨 UI Design

Modern interfaces focused on usability and clean aesthetics.

</td>

</tr>

</table>

---

# 🌱 Currently Learning

<div align="center">

| Technology | Progress |
|------------|----------|
| React | 🟢🟢🟢⚪⚪ |
| TypeScript | 🟢🟢⚪⚪⚪ |
| Node.js | 🟢🟢🟢🟢⚪ |
| Express | 🟢🟢🟢⚪⚪ |
| UI/UX | 🟢🟢🟢🟢⚪ |
| Software Architecture | 🟢🟢🟢⚪⚪ |

</div>

---

# 🌟 Core Values

✔ Clean Code

✔ Continuous Learning

✔ Problem Solving

✔ Team Collaboration

✔ Performance First

✔ User Experience

✔ Creativity

✔ Responsibility

---

# 📌 Development Philosophy

> "Good software is not only functional — it should also be intuitive, maintainable and enjoyable to use."

# 🚀 Featured Projects

<br>

<table>

<tr>

<td width="50%">

## 🏥 Carolina Plus

<img src="https://img.shields.io/badge/Status-In%20Development-22C55E?style=for-the-badge"/>

A comprehensive pharmacy management system designed to optimize business operations through automation and modern technologies.

### ✨ Features

- 💊 Inventory Management
- 🛒 Sales & Billing
- 👥 Customer Management
- 📦 Supplier Management
- 🔔 Smart Notifications
- 📊 Reports & Statistics
- 🎁 Promotions Module
- 🔒 User Authentication

### 🛠 Built With

<p>

<img src="https://skillicons.dev/icons?i=html,css,js,firebase"/>

</p>

</td>

<td width="50%">

## 💼 Business Management Systems

Development of custom management systems for businesses.

### Systems include

- Inventory
- Billing
- Customers
- Reports
- Sales
- Products
- Dashboards
- Administration

### Technologies

<p>

<img src="https://skillicons.dev/icons?i=html,css,js,nodejs,mysql"/>

</p>

</td>

</tr>

</table>

---

# 🎯 Areas of Expertise

<table>

<tr>

<td>

### 🌐 Frontend

- Responsive Design
- HTML5
- CSS3
- JavaScript
- Bootstrap
- React

</td>

<td>

### ⚙ Backend

- Node.js
- Express
- PHP
- REST APIs
- Authentication
- CRUD Systems

</td>

</tr>

<tr>

<td>

### 🗄 Database

- Firebase
- Firestore
- MySQL
- MongoDB

</td>

<td>

### 🎨 Design

- UI Design
- UX Design
- Wireframes
- Responsive Interfaces
- Prototyping

</td>

</tr>

</table>

---

# 💡 What Makes Me Different

✔ Focus on clean and maintainable code

✔ User-centered interface design

✔ Modern and responsive layouts

✔ Efficient database design

✔ Organized project structure

✔ Continuous learning mindset

✔ Passion for solving real-world problems

✔ Attention to detail

---

# 🧩 Development Workflow

```text
Idea
 │
 ▼
Planning
 │
 ▼
UI Design
 │
 ▼
Development
 │
 ▼
Testing
 │
 ▼
Deployment
 │
 ▼
Continuous Improvement
```

---

# 📚 Current Focus

🟢 Full Stack Development

🟢 Modern JavaScript

🟢 React Ecosystem

🟢 Firebase Services

🟢 REST API Design

🟢 Software Architecture

🟢 UI / UX Improvements

🟢 Performance Optimization

---

# 🎓 Professional Goals

🎯 Become a Senior Full Stack Developer

🎯 Build scalable web applications

🎯 Master cloud technologies

🎯 Learn advanced software architecture

🎯 Contribute to impactful projects

🎯 Keep improving every day

---

# 📖 Development Principles

> **Write code that people can understand, not just computers.**

<br>

> **Every project is an opportunity to learn something new.**

<br>

> **Good software begins with understanding the user's needs.**

---

# ⚡ Fun Facts

💻 I enjoy building complete web applications from scratch.

📚 Learning new technologies is part of my daily routine.

🎨 I like combining development with modern UI design.

🧩 I enjoy solving programming challenges.

🚀 I believe technology can simplify people's lives.

☕ My favorite debugging method is patience and logical thinking.

---

# 🌍 Languages

| Language | Level |
|----------|---------|
| Spanish 🇨🇴 | Native |
| English 🇺🇸 | Intermediate |

---

# 🤝 Open To

✅ Freelance Projects

✅ Full-Time Opportunities

✅ Remote Work

✅ Team Collaboration

✅ Learning New Technologies

✅ Open Source Contributions

---

# 💬 Favorite Quote

> *"The best way to predict the future is to create it."*

# 🧰 Developer Toolkit

<div align="center">

<table>

<tr>

<td align="center" width="120">

<img src="https://skillicons.dev/icons?i=vscode"/>

**VS Code**

</td>

<td align="center" width="120">

<img src="https://skillicons.dev/icons?i=git"/>

**Git**

</td>

<td align="center" width="120">

<img src="https://skillicons.dev/icons?i=github"/>

**GitHub**

</td>

<td align="center" width="120">

<img src="https://skillicons.dev/icons?i=firebase"/>

**Firebase**

</td>

</tr>

<tr>

<td align="center">

<img src="https://skillicons.dev/icons?i=mysql"/>

**MySQL**

</td>

<td align="center">

<img src="https://skillicons.dev/icons?i=nodejs"/>

**Node.js**

</td>

<td align="center">

<img src="https://skillicons.dev/icons?i=figma"/>

**Figma**

</td>

<td align="center">

<img src="https://skillicons.dev/icons?i=photoshop"/>

**Photoshop**

</td>

</tr>

</table>

</div>

---

# ⚙ Development Environment

<table>

<tr>

<td>

💻 **Operating System**

Windows 11

</td>

<td>

🖥 **Code Editor**

Visual Studio Code

</td>

</tr>

<tr>

<td>

🌱 **Version Control**

Git & GitHub

</td>

<td>

☁ **Cloud Services**

Firebase

</td>

</tr>

<tr>

<td>

🗄 **Database**

MySQL • Firestore

</td>

<td>

🌐 **Browsers**

Chrome • Edge

</td>

</tr>

</table>

---

# 📌 My Development Process

```text
📋 Planning
      │
      ▼
🎨 UI / UX Design
      │
      ▼
💻 Development
      │
      ▼
🧪 Testing
      │
      ▼
🚀 Deployment
      │
      ▼
🔄 Continuous Improvement
```

---

# 🎯 My Priorities

✔ Writing clean and maintainable code

✔ Building responsive interfaces

✔ Creating intuitive user experiences

✔ Designing scalable systems

✔ Learning modern technologies

✔ Solving real business problems

---

# 🌱 Beyond Coding

When I'm not programming, I enjoy:

🎮 Video games

🎸 Learning new skills

📚 Exploring new technologies

🎨 UI inspiration

💡 Building personal projects

---

# 📫 Contact Me

<div align="center">

| 📧 Email | 🌍 Location |
|-----------|-------------|
| jethzibelarr@gmail.com | Colombia 🇨🇴 |

</div>

---

# 💚 Thanks For Visiting

<div align="center">

### Thanks for stopping by!

I'm always excited to learn, collaborate and build meaningful software.

If you like my work, feel free to explore my repositories.

⭐ Happy Coding!

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=22C55E&height=120&section=footer"/>

</div>
